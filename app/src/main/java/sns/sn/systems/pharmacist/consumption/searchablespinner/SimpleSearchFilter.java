...
public class SimpleSearchFilter<T extends Searchable> extends BaseFilter {
	...
	protected Filter.FilterResults performFiltering(CharSequence chars) {
		...
		for (T item : mItems) {
			if (item.getPatientName().toLowerCase().contains(filterSeq)) {
				filteredItems.add(new Pair<>(item, filterSeq.length()));
			} else if (mCheckLCS) {
				int lcsLength = StringsHelper.lcs(item.getPatientName(), filterSeq).length();
				if (lcsLength > item.getPatientName().length() * mAccuracyPercentage) {
